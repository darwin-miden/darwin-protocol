//! Deploy + verify the v6 controller package on Miden testnet.
//!
//! Two phases:
//!
...
//!
//! Usage:
//!     cargo run -p darwin-protocol-account --bin deploy_v6 -- \
//!         --masp /tmp/darwin-v6.masp
//!
//! Prereqs:
...
fn parse_args() -> (PathBuf, bool) {
    ...
    (
        masp.unwrap_or_else(|| PathBuf::from("/tmp/darwin-v6.masp")),
        deploy,
    )
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Loading v6 controller package from {}", masp_path.display());
    ...
    println!();
    println!("🆕  v6 controller account built");
    ...
    println!();
    println!("✓ v6 controller registered");
    println!("    id (hex)          : {}", account.id().to_hex());
