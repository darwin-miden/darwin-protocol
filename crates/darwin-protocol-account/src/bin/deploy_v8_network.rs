...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let metadata = AccountComponentMetadata::new("darwin-basket-controller-v8-network-account");
    ...
    // v8 — Network Account auth. NO private key — the Miden network
    // itself auto-executes txs against this account when notes whose
    // script root is in the allowlist arrive. This is Solution B
    // ("Network Accounts") of the grant proposal: zero backend, zero
    // signature required from any human operator.
    //
    // The allowlist for the PoC is populated at deploy time via
    // DARWIN_ALLOWED_SCRIPT_ROOTS (comma-separated hex Digest, 64 hex
    // chars each). Concretely we pass the atomic_deposit_note_v3
    // NoteScript root — computed offline via
    // `darwin_protocol_account::note_script_roots::atomic_deposit_v3()`
    // once we wire it up.
    //
    // TODO(post-PoC): also allowlist the atomic_redeem_note root so the
    // network-account can auto-drive redeem flows.
    let allowed_hex = std::env::var("DARWIN_ALLOWED_SCRIPT_ROOTS")
        .map_err(|_| "DARWIN_ALLOWED_SCRIPT_ROOTS env required (comma-separated 64-hex Digest values)".to_string())?;
    let mut allowed = BTreeSet::new();
    for h in allowed_hex.split(',') {
        let s = h.trim();
        if s.is_empty() { continue; }
        let w = Word::parse(s)
            .map_err(|e| format!("parse Word from {s:?}: {e}"))?;
        allowed.insert(NoteScriptRoot::from_raw(w));
    }
    if allowed.is_empty() {
        return Err("allowlist is empty — nothing would ever auto-consume".into());
    }
    println!();
    println!("Network-account allowlist ({} script root(s)):", allowed.len());
    for d in &allowed {
        println!("  - {d}");
    }
    let auth_component = AuthNetworkAccount::with_allowed_notes(allowed)
        .map_err(|e| format!("AuthNetworkAccount::with_allowed_notes: {e}"))?;
