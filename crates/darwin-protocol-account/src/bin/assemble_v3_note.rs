//! Sanity-check that `atomic_deposit_note_v3.masm` assembles cleanly
//! against the same transaction-kernel assembler the v5 controller +
//! production deploy use.
//!
//! Useful as a CI guard whenever the v3 note format or the v5 MAST
//! roots change.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let program = TransactionKernel::assembler()
        ...
        .assemble_program(darwin_notes::ATOMIC_DEPOSIT_NOTE_V3_MASM)?;

    println!("✓ atomic_deposit_note_v3 assembles");
